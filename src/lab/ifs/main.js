...
const app = new Vue({
  ...
  watch: {
    fractal: {
      handler: function (ifs) {
        this.render()
      },
      deep: true
    },
    playground: function (on) {
      if (on) {
        document.getElementById('content').classList.add('playground');
      } else {
        document.getElementById('content').classList.remove('playground');
      }
    }
  },
  methods: {
