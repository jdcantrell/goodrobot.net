...
// range examples
var range1 = new Canvas('range1');
var range2 = new Canvas('range2');
var range3 = new Canvas('range3');
...
worker.render().then((imageData) => { range1.setImageData(imageData); })
  .then(() => {
    worker.setRange(
...
itWorker.render().then((imageData) => { low.setImageData(imageData) })
  .then(() => {
    itWorker.setMaxIterations(255);
    return itWorker.render();
  })
  .then((imageData) => { med.setImageData(imageData) })
  .then(() => {
    itWorker.setMaxIterations(1000);
    return itWorker.render();
  })
  .then((imageData) => { high.setImageData(imageData) })

//color examples
var colors1 = new Canvas('colors1');
var colors2 = new Canvas('colors2');
var colors3 = new Canvas('colors3');
size = colors1.size();
...
cWorker.render().then((imageData) => { colors1.setImageData(imageData) })
  .then(() => {
    cWorker.setColors([0, 5, 0], [0, 255, 0], [18, 42, 69]);
    return cWorker.render();
  })
  .then((imageData) => { colors2.setImageData(imageData) })
  .then(() => {
    cWorker.setColors([0, 0, 0], [255, 255, 255]);
    return cWorker.render();
  })
  .then((imageData) => { colors3.setImageData(imageData) })
